...
class TeamsMessagingExtensionsSearchAuthConfigBot extends TeamsActivityHandler {
    ...
    async handleTeamsMessagingExtensionQuery(context, query) {
        ...
        if (!userSettings || userSettings.includes('profile')) {
            console.log('----------------', Number(query.state))
            ...
            if (!tokenResponse || !tokenResponse.token) {
                // There is no token, so the user has not signed in yet.
                console.log('---------------------- tokenIsExchangeable type of userid: ', context.activity.from.id, typeof(context.activity.from.id), context.activity.channelId, typeof(context.activity.channelId))
                // Retrieve the OAuth Sign in Link to use in the MessagingExtensionResult Suggested Actions
                const signInLink = await context.adapter.getSignInLink(
                    context,
                    this.connectionName
    ...
    async tokenIsExchangeable(context) {
        console.log('-------------------- tokenIsExchangeable')
